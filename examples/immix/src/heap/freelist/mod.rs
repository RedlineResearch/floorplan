...
impl FreeListSpace {
    ...
    pub fn mark(&mut self, _obj: CellAddr) {
        
    }
    
    pub fn alloc(&mut self, size: usize, align: usize) -> Option<ObjectAddr> {
        if self.used_bytes + size > self.size {
            None
        } else {
            let ret = self::aligned_alloc::aligned_alloc(BYTES_IN_WORD + size, align);
            
            let addr = CellAddr::from_ptr::<()>(ret);
            
            self.current_nodes.push_front(Box::new(FreeListNode{id: self.node_id, start: addr, size: size, mark: NodeMark::FreshAlloc}));
            self.node_id += 1;
            self.used_bytes += size;
           
            addr.cell_size().store_word(size);
            Some(addr.object())
        }
    }
    ...
    pub fn sweep(&mut self) {
        let (new_nodes, new_used_bytes) = {
            ...
            while !nodes.is_empty() {
                ...
                match node.mark {
                    NodeMark::Live => {
                        //let refBitsA : RefBitsAddr =
                        //    Word2RefBits::map_getAddr(
                        //        self.start.get_first_word(),
                        //        self.alloc_map.toStart,
                        //        WordAddr::from_usize(line_addr.as_usize() + j));
                        //check_expect_pc!(refBitsA.as_usize(), 
...
#[inline(never)]
pub fn alloc_large(size: usize, align: usize, mutator: &mut immix::ImmixMutatorLocal, space: Arc<RwLock<FreeListSpace>>) -> ObjectAddr {
    loop {
        mutator.yieldpoint();
